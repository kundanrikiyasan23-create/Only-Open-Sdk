 ...
 //   WebView.setDataDirectorySuffix(getUserId() + ":" + packageName + ":" + processName);
    ...
    try {
        if (!isInit()) {
            ...
            ContextCompat.fix(
                    (Context) BRActivityThread.get(BlackBoxCore.mainThread()).getSystemContext()
            );
...
public synchronized void handleBindApplication(String str, String str2) {
    try {
        ...
        // ✅ FIXED WebView data directory (Android 9+)
        ...
        if (BuildCompat.isN() && targetSdk < 24) {
            StrictModeCompat.disableDeathOnFileUriExposure();
        }
